    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          <div className="flex flex-col min-w-0">
              ...
              <AnimatePresence>
                {user.isFocusing && (
                  <motion.div
                    ...
                    className="flex items-center space-x-1 px-1.5 py-0.5 rounded-full bg-accent/20 border border-accent/30"
                  >
                    <div className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" />
                    <span className="text-[7px] font-black text-accent uppercase tracking-tighter">{t('leaderboard.focusing', state.language)}</span>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="flex items-center space-x-3 mt-1 shrink-0">
              ...
              {(() => {
                const rating = getIntegrityRating(user.integrityScore ?? 90, state.language);
                return (
                  <div 
        ...
        <div className="flex items-center space-x-3 mt-1">
          <span className="text-xs font-mono text-accent">{t('leaderboard.lvl', state.language, { level: user.level })}</span>
...
const LeaderboardScreen = ({ state }: LeaderboardScreenProps) => {
  ...
  const allUsers = useMemo(() => {
    ...
    // Sort by Level first, then OVR, then totalXp DESC
    finalUsers.sort((a, b) => {
      if (b.level !== a.level) {
        return b.level - a.level;
      }

      const aOvr = a.ovr || (a.level ? calculateOVR(a as any, null).ovr : 0);
      const bOvr = b.ovr || (b.level ? calculateOVR(b as any, null).ovr : 0);
    ...
    return finalUsers;
  }, [users, state.userId, state.username, state.level, state.xp, state.equippedFrame, state.equippedTitle, state.profilePicture, state.streak, state.badges, state.unlockedFrames, state.missionsCompleted, state.isProfilePublic, state.integrityScore, state.baseStats, activeUserEmail]);
            ...
            >
              ...
              <div className="h-24 w-full bg-gradient-to-t from-surface to-surface-hover rounded-t-xl mt-2 border-t-2 border-slate-300/50 flex flex-col items-center justify-between py-2">
                <div className="flex flex-col items-center">
                  {top50Users[1] && (
                    <div 
                      className={`w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-black border border-white/10 shrink-0 mb-1 ${getIntegrityRating(top50Users[1].integrityScore ?? 90, state.language).glow}`}
                      style={{ color: getIntegrityRating(top50Users[1].integrityScore ?? 90, state.language).color, backgroundColor: `${getIntegrityRating(top50Users[1].integrityScore ?? 90, state.language).color}20` }}
                    >
                      {getIntegrityRating(top50Users[1].integrityScore ?? 90, state.language).letter}
                    </div>
                  )}
                  <div className="flex items-center space-x-1">
                    <span className="text-[7px] font-black text-white/40 uppercase">OVR</span>
                    <span className="text-[10px] font-black text-primary">{top50Users[1]?.ovr || (top50Users[1] ? calculateOVR(top50Users[1] as any, null).ovr : 0)}</span>
                  </div>
                </div>
                <span className="text-xs font-mono text-secondary">{t('leaderboard.lvl', state.language, { level: (top50Users[1]?.level || 0) })}</span>
              </div>
            </div>
            ...
            >
              ...
              <div className="h-32 w-full bg-gradient-to-t from-surface to-surface-hover rounded-t-xl mt-2 border-t-4 border-yellow-400/50 flex flex-col items-center justify-between py-3">
                <div className="flex flex-col items-center">
                  {top50Users[0] && (
                    <div 
                      className={`w-6 h-6 rounded-md flex items-center justify-center text-xs font-black border border-white/10 shrink-0 mb-1 ${getIntegrityRating(top50Users[0].integrityScore ?? 90, state.language).glow}`}
                      style={{ color: getIntegrityRating(top50Users[0].integrityScore ?? 90, state.language).color, backgroundColor: `${getIntegrityRating(top50Users[0].integrityScore ?? 90, state.language).color}20` }}
                    >
                      {getIntegrityRating(top50Users[0].integrityScore ?? 90, state.language).letter}
                    </div>
                  )}
                  <div className="flex items-center space-x-1">
                    <span className="text-[8px] font-black text-white/40 uppercase">OVR</span>
                    <span className="text-xs font-black text-yellow-400">{top50Users[0]?.ovr || (top50Users[0] ? calculateOVR(top50Users[0] as any, null).ovr : 0)}</span>
                  </div>
                </div>
                <span className="text-sm font-mono font-bold text-yellow-400">{t('leaderboard.lvl', state.language, { level: (top50Users[0]?.level || 0) })}</span>
              </div>
            </div>
            ...
            >
              ...
              <div className="h-20 w-full bg-gradient-to-t from-surface to-surface-hover rounded-t-xl mt-2 border-t-2 border-amber-700/50 flex flex-col items-center justify-between py-2">
                <div className="flex flex-col items-center">
                  {top50Users[2] && (
                    <div 
                      className={`w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-black border border-white/10 shrink-0 mb-1 ${getIntegrityRating(top50Users[2].integrityScore ?? 90, state.language).glow}`}
                      style={{ color: getIntegrityRating(top50Users[2].integrityScore ?? 90, state.language).color, backgroundColor: `${getIntegrityRating(top50Users[2].integrityScore ?? 90, state.language).color}20` }}
                    >
                      {getIntegrityRating(top50Users[2].integrityScore ?? 90, state.language).letter}
                    </div>
                  )}
                  <div className="flex items-center space-x-1">
                    <span className="text-[7px] font-black text-white/40 uppercase">OVR</span>
                    <span className="text-[10px] font-black text-primary">{top50Users[2]?.ovr || (top50Users[2] ? calculateOVR(top50Users[2] as any, null).ovr : 0)}</span>
                  </div>
                </div>
                <span className="text-xs font-mono text-secondary">{t('leaderboard.lvl', state.language, { level: (top50Users[2]?.level || 0) })}</span>
              </div>
            </div>
          </div>
        ...
        <div className="absolute bottom-[72px] left-0 right-0 px-6 pb-2 pt-2 bg-gradient-to-t from-background via-background to-transparent z-40">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                <div className="flex flex-col min-w-0">
                  ...
                  <div className="flex items-center space-x-3 mt-1 shrink-0">
                    ...
                    {(() => {
                      const rating = getIntegrityRating(currentUserData.integrityScore ?? 90, state.language);
                      return (
                        <div 
                          className={`w-4 h-4 rounded-md flex items-center justify-center text-[8px] font-black border border-white/10 ${rating.glow}`}
                          style={{ color: rating.color, backgroundColor: `${rating.color}20` }}
              ...
              <div className="flex items-center space-x-3 mt-1">
                <span className="text-xs font-mono text-accent">{t('leaderboard.lvl', state.language, { level: currentUserData.level })}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center p-4 bg-black/60 backdrop-blur-sm pb-24">
          ...
          >
            ...
            <div className="flex flex-col items-center mb-6">
              ...
              <p className="text-sm text-secondary">{t('profile.level', state.language)} {selectedActionUser.level}</p>
            </div>
            ...
            {selectedUser.isProfilePublic === false ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="w-full mt-6">
                  <h4 className="text-xs font-mono uppercase tracking-widest text-secondary mb-3 px-1">{t('profile.badges', state.language)}</h4>
                        ...
                        if ((selectedUser.badgesCount || 0) > 0) {
                          return (
                            <div className="p-4 rounded-xl border bg-white/5 border-white/10 flex flex-col items-center justify-center text-center col-span-2">
                              ...
                              <span className="text-xs font-bold text-primary">{t('leaderboard.badges_earned', state.language, { count: selectedUser.badgesCount || 0 })}</span>
                              <span className="text-[10px] text-secondary mt-1 italic">{t('leaderboard.syncing_details', state.language)}</span>
                            </div>
                          );
                        }
                        return <p className="text-[10px] text-secondary/50 italic px-1 col-span-2">{t('leaderboard.no_badges', state.language)}</p>;
                      }
