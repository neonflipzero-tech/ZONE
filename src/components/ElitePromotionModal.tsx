    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className={`relative w-full max-w-md bg-background border ${isFlashSale ? 'border-yellow-400/50' : 'border-accent/30'} rounded-[2.5rem] overflow-hidden shadow-[0_0_80px_rgba(242,125,38,0.25)] max-h-[95vh] flex flex-col`}
        >
          {/* Animated background glow */}
          <div className={`absolute top-0 left-0 right-0 h-48 bg-gradient-to-br ${isFlashSale ? 'from-yellow-400/30 via-orange-500/20' : 'from-accent/30 via-purple-500/20'} to-transparent pointer-events-none`} />
          
          {isFlashSale && (
            <div className="absolute top-0 left-0 right-0 py-2 bg-yellow-400 flex items-center justify-center space-x-2 z-30">
          ...
          <div className="relative flex-1 overflow-y-auto no-scrollbar pt-12 px-6 pb-8 flex flex-col items-center text-center">
            <motion.div 
              ...
              className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${isFlashSale ? 'from-yellow-400 via-orange-400 to-red-600' : 'from-accent via-orange-400 to-purple-600'} p-0.5 mb-4 shadow-[0_0_30px_rgba(242,125,38,0.3)] shrink-0`}
            >
              <div className="w-full h-full bg-background rounded-2xl flex items-center justify-center">
                <Crown className={`w-8 h-8 ${isFlashSale ? 'text-yellow-400' : 'text-accent'}`} />
              </div>
            </motion.div>
            ...
            <div className="mb-6 shrink-0">
              ...
              <div className={`h-1 w-12 ${isFlashSale ? 'bg-yellow-400' : 'bg-accent'} mx-auto rounded-full mb-3`} />
            ...
            {/* Plan Selection */}
            <div className={`w-full ${isFlashSale ? 'flex justify-center' : 'grid grid-cols-1 gap-2'} mb-6 shrink-0`}>
              {plans.map((plan) => (
                <button
                  key={plan.id}
                  onClick={() => setSelectedPlan(plan.id as any)}
                  className={`relative p-3 rounded-2xl border-2 transition-all text-left flex flex-col ${isFlashSale ? 'w-full max-w-[240px]' : ''} ${
                    selectedPlan === plan.id
                      ? (isFlashSale ? 'bg-yellow-400/10 border-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.2)]' : 'bg-accent/10 border-accent shadow-[0_0_20px_rgba(242,125,38,0.2)]')
                      : 'bg-white/[0.03] border-white/5 hover:border-white/10'
                  }`}
                >
                  {selectedPlan === plan.id && (
                    <div className={`absolute top-2 right-2 ${isFlashSale ? 'bg-yellow-400' : 'bg-accent'} rounded-full p-0.5`}>
                      <Check className="w-3 h-3 text-black" />
                    </div>
                  )}
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-[9px] uppercase tracking-widest font-black ${
                      selectedPlan === plan.id ? (isFlashSale ? 'text-yellow-400' : 'text-accent') : 'text-secondary'
                    }`}>
                      {plan.name}
                    </span>
                    {plan.id === 'yearly' && (
            ...
            <div className="w-full bg-white/[0.03] rounded-3xl p-5 mb-6 border border-white/5 backdrop-blur-sm shrink-0">
              <ul className="grid grid-cols-1 gap-3 text-left">
                {features.map((feature, i) => (
                  ...
                  >
                    <div className={`flex-shrink-0 ${isFlashSale ? 'bg-yellow-400/20' : 'bg-accent/20'} p-1.5 rounded-xl`}>
                      <Check className={`w-3 h-3 ${isFlashSale ? 'text-yellow-400' : 'text-accent'}`} />
                    </div>
            ...
            <div className="w-full space-y-3 shrink-0">
              <button
                ...
                disabled={loading}
                className={`group relative w-full py-4 rounded-2xl font-black text-lg ${isFlashSale ? 'bg-gradient-to-r from-yellow-400 via-orange-500 to-red-600' : 'bg-gradient-to-r from-accent via-orange-500 to-purple-600'} text-white shadow-lg hover:scale-[1.02] transition-all flex items-center justify-center space-x-3 disabled:opacity-70 disabled:cursor-not-allowed overflow-hidden`}
              >
